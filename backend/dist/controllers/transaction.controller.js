...
async function getTransactions(req, res) {
    try {
        ...
        const { type, start, end, q, categoryId, paymentMethod, min, max } = req.query;
...
async function getDashboardStats(req, res) {
    try {
        ...
        if (req.query.categoryId)
            totalsQB.andWhere('t.categoryId = :categoryId', { categoryId: req.query.categoryId });
        if (req.query.paymentMethod)
            totalsQB.andWhere('t.paymentMethod = :paymentMethod', { paymentMethod: req.query.paymentMethod });
        ...
        const catQB = repo.createQueryBuilder('t')
            ...
            .andWhere('t.date BETWEEN :start AND :end', { start: startBoundary, end: endBoundary });
        if (req.query.categoryId)
            catQB.andWhere('t.categoryId = :categoryId', { categoryId: req.query.categoryId });
        if (req.query.paymentMethod)
            catQB.andWhere('t.paymentMethod = :paymentMethod', { paymentMethod: req.query.paymentMethod });
        ...
        const trendQB = repo.createQueryBuilder('t')
            ...
            .andWhere('t.date BETWEEN :start AND :end', { start: startBoundary, end: endBoundary });
        if (req.query.type && ['income', 'expense'].includes(req.query.type))
            trendQB.andWhere('t.type = :tfilter', { tfilter: req.query.type });
        if (req.query.categoryId)
            trendQB.andWhere('t.categoryId = :categoryId', { categoryId: req.query.categoryId });
        if (req.query.paymentMethod)
            trendQB.andWhere('t.paymentMethod = :paymentMethod', { paymentMethod: req.query.paymentMethod });
        ...
        rawTrend.forEach((r) => byMonth.set(r.month, { income: Number(r.income), expense: Number(r.expense) }));
        const trend = {
            months,
...
async function exportCsv(req, res) {
    try {
        ...
        const lines = data.map(t => [t.type, t.category?.name || '', t.amount, t.date, t.note || '', t.paymentMethod || '']
            .map(escape)
            .join(',')).join('\n');
...
async function importCsv(req, res) {
    try {
        ...
        const catRepo = db_1.AppDataSource.getRepository(Category_1.Category);
        let imported = 0;
        const errors = [];
        let createdCategories = 0;
        const findOrCreateCategory = async (name, type) => {
            ...
            if (!cat) {
                ...
                cat = catRepo.create({ userId, name, type: type });
        ...
        for (let r = 0; r < rows.length; r++) {
            const parts = rows[r];
            const type = (parts[iType] || '').trim().toLowerCase();
            ...
            const tx = txRepo.create({ userId, type: type, amount, date, categoryId: categoryId, note, paymentMethod });
